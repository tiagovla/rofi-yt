 ...
 * gcc -std=c99 -Wall -o cairo_jpg -DCAIRO_JPEG_MAIN `pkg-config cairo libjpeg
 * --cflags --libs` cairo_jpg.c
 *
...
cairo_status_t cairo_image_surface_write_to_jpeg_mem(cairo_surface_t *sfc, unsigned char **data, size_t *len,
                                                     int quality) {
...
#ifdef LIBJPEG_TURBO_VERSION
        row_pointer[0] =
            cairo_image_surface_get_data(sfc) + (cinfo.next_scanline * cairo_image_surface_get_stride(sfc));
#else
        unsigned char row_buf[3 * cinfo.image_width];
        pix_conv(row_buf, 3,
                 cairo_image_surface_get_data(sfc) + (cinfo.next_scanline * cairo_image_surface_get_stride(sfc)), 4,
                 cinfo.image_width);
...
static cairo_status_t cj_write(void *closure, const unsigned char *data, unsigned int length) {
    return write((intptr_t)closure, data, length) < length ? CAIRO_STATUS_WRITE_ERROR : CAIRO_STATUS_SUCCESS;
}
 ...
 */
cairo_status_t cairo_image_surface_write_to_jpeg_stream(cairo_surface_t *sfc, cairo_write_func_t write_func,
...
cairo_surface_t *cairo_image_surface_create_from_jpeg_mem(void *data, size_t len) {
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
        unsigned char *row_address =
            cairo_image_surface_get_data(sfc) + (cinfo.output_scanline * cairo_image_surface_get_stride(sfc));
